                  ...
                  .on('init', function () {
                    ...
                    if (roots && roots.length) {
                      roots[0].setExpanded(false).done(function () {
                        roots[0].setExpanded(expanded).done(function () {
                          // console.error("auto-expand done");
                        }).fail(function () {
                          console.error("auto-expand fail");
                        }).always(function () {
                          // console.error("auto-expand always");
                        });
                      });
                    }
                    ...
                    $.each(roots, function(n,x){
                      if (n != 0) {
                        if (x.expanded && !x.children) {
                          if (DEBUG) {
                            console.log("EXPOOPOO! " + n + " " + x.title + " thinks its expanded but it has no children");
                            console.dir(x);
                          }
                          x.setExpanded(false);
                        }
                          ...
                          updateHandler: function (blob) {
                            ...
                            if (Drupal.settings.kmapsSolr.getState().assetTypeList) {
                              ...
                              if (assetTypes.length === 0 || assetTypes[0] === 'all' ) {
                                ...
                                if (DEBUG) console.log("defaultAssetType === " + defaultAssetType);
