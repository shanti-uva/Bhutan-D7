...
( function() {
	...
	CKEDITOR.cleanWord = function( mswordHtml, editor ) {
		console.log("In custom clean up from word");
		...
		filter = new CKEDITOR.htmlParser.filter( {
			...
			elements: {
				...
				'div': function( element ) {
					...
					// Isolate and mark the endnotes div
					var edn = element.find(function(chld) {
						if (chld.attributes) {
							var id = chld.attributes['id'];
							if (id && id.indexOf('edn') > -1) {
								return true;
							}
						}
						return false;
					...
					if (edn.length > 0 ) {
						var endnotes = new CKEDITOR.htmlParser.element('div', {'class': 'endnotes'});
						for (var n in edn) {
							var ent = edn[n]
							ent.addClass('endnote');
							console.log(edn[n].children[0].children[0]);
							// strip out p element with styling. It's one child is a span
							ent.children = ent.children[0].children;
							endnotes.add(ent);
						}
						element.replaceWith(endnotes);
					}
				},
				'i': function( element ) {
					element.name = "em";
