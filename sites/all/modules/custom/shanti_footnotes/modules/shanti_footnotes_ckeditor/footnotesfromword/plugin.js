 ...
 *
 *  This file has been modified by Than to work properly with Shanti Texts site. Do not upgrade without comparing.
 *  ndg, 4/17/19
 */
        ...
        editor.on( 'paste', function( evt ) {
            ...
            contents.find('a[name]').each(function(){
                ...
                if ($this.attr('name').match(/^_ftnref/)) {
                    ...
                    $footnote_p.find('a[href="#' + $this.attr('name') + '"]').remove();  // Added by ndg8f bec. following line does not work
                    ...
                    // Added by ndg8f to replace update html markup for italics and bold
                    footnote_text = footnote_text.replace(/<i>/g, '<em>').replace(/<\/i>/g, '</em>');
                    footnote_text = footnote_text.replace(/<b>/g, '<strong>').replace(/<\/b>/g, '</strong>');
            ...
            // Added by ndg8f. Remove original MS footnote div
            var ftndiv = contents.find('div#ftn1').eq(0).parent('div');
            ftndiv.remove();
            ...
            if ((/<section.*class="footnotes"/).test(data)) {
                ...
                data += '<section class="footnotes"><header>' + header_els[0] + header_title + header_els[1] + '</header><ol>' + footnotes + '</ol></section>';
            }
