...
Drupal.ajax && (Drupal.ajax.prototype.eventResponse = function (element, event) {
  ...
  // Save the original data the first time, to be used if the request fails, because
  // Drupal reuses the entire ajax object & state.
  if (!ajax.options._originalData) {
    ajax.options._originalData = JSON.stringify(ajax.options.data);
  }
  else {
    ajax.options.data = JSON.parse(ajax.options._originalData);
  }
