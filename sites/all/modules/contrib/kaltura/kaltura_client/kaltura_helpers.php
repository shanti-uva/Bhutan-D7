...
class KalturaHelpers {
  ...
  function getKalturaClient($isAdmin = FALSE, $privileges = NULL, $expiry = 86400) {
    ...
    if ($isAdmin) {
      $result = $kalturaClient->session->start($adminSecret, $session_user->id, KalturaSessionType::ADMIN, $partnerId, $expiry, $privileges);
    }
    else {
      $result = $kalturaClient->session->start($secret, $session_user->id, KalturaSessionType::USER, $partnerId, $expiry, $privileges);
    }
    $len = strlen($result);
