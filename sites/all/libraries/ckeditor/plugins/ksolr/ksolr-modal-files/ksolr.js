...
function ksSolr()																// CONSTRUCTOR
{
	...
	this.pageSize = 100;
	this.startDoc=0;
	// set the solrUrl variable for this object based on host
	this.solrUrl = 'https://ss395824-us-east-1-aws.measuredsearch.com/solr/kmassets/select';   
	var host = window.location.host;
	if (host.indexOf('.dd') > -1 || window.location.host.indexOf('-dev') > -1 ) {
        this.solrUrl = 'https://ss251856-us-east-1-aws.measuredsearch.com/solr/kmassets_dev/select';
	} else if (host.indexOf('-predev.') > -1 ) {
	    this.solrUrl = 'https://ss251856-us-east-1-aws.measuredsearch.com/solr/kmassets_predev/select';
	} else if (host.indexOf('-stage') > -1 ) {
        this.solrUrl = 'https://ss395824-us-east-1-aws.measuredsearch.com/solr/kmassets_stage/select';
	}
}

ksSolr.prototype.ImportSolrDialog=function(callback, mode)				// SOLR IMPORTER DIALOG
{
	var i;
	...
	var listtypes = ['Sources', 'Texts'];  // Types that should show lists by default
	...
	str+='<p class="result-summary"><span class="rangediv">Showing results <span class="start">1</span> - <span class="end">' + this.maxDocs + '</span> of </span>';
	str+="<span id='numItemsFound'>No</span> items found</p>";        // Number of items;
	...
	str+='<div class="dialogctrl"><div id="prev" class="ks-bs">Previous</div><div id="next" class="ks-bs">Next</div></div>';
	...
	LoadCollection();															// Load 1st collection
 	...
 	$("#mdType").on("change", function() {											// ON CHANGE COLLECTION
            ...
            _this.startDoc=0;           
            // If asset type is AV, Images, or Visuals, (i.e. not Sources or Texts) use Grid View
			var current_view = $("#mdView").val();
			if (listtypes.indexOf(_this.type) == -1) {
			    if (current_view == 'List') { 
			        $("#mdView").val('Grid'); 
			        _this.view = 'Grid';
		        }
		    // Otherwise (i.e. if type is Sources or Texts) use List View
	...
	$("#mdFilterSubject").on("click", function() {									// ON CLICK PLACE FILTER **THAN**
 	...
 	function LoadCollection() {													// LOAD COLLECTION FROM SOLR
		...
		var url= _this.solrUrl +"?q="+search + 
   				 "&fl=*&wt=json&json.wrf=?&rows="+_this.maxDocs+"&limit="+_this.maxDocs + "&start=" + _this.startDoc;
...
ksSolr.prototype.FormatSolrItems=function(data, sortBy)							// SHOW SOLR ITEMS
{
		...
		for (i=0;i<data.response.docs.length;++i) {									// For each doc returned
			...
			if ((r.asset_type == "picture") || (r.asset_type == "image")) {			// An image
				if (r.url_huge)			o.ajax=r.url_huge.replace(/dev\-/,"");		// If huge, set url and remove 'dev-' prefix
                 ...
                 else if (r.url_thumb)  o.ajax=r.url_thumb;    // Else use url_thumb
	...
	$("#numItemsFound").text(data.response.numFound);										// Show number of results
...
{	
	...
	for (i=0;i<this.data.length;++i) {												// For each doc returned
		...
		str+="</div><span style='color:#27ae60'>"+(_this.startDoc + i+1)+". </span>";				// Add pic num
